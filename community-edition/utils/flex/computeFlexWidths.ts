...
type Params = {
  ...
  maxWidths?: (number | null | undefined)[];
  minWidths?: (number | null | undefined)[];
};

export default ({
  flexes,
  availableSize,
  maxWidths = EMPTY_ARRAY,
  minWidths = EMPTY_ARRAY,
}: Params): (number | null)[] => {
  flexes = [...flexes];
  ...
  const result: (number | null)[] = flexes.map(_ => null);
  ...
  const lastValidFlexIndex = flexes.reduce(
    (acc: number, flex: number, index: number) => (flex ? index : acc),
    -1
  );

  flexes.forEach((flex: number, index: number) => {
    if (!flex) {
      return;
    }
