...
export const createGroupItem = (group, pivot?: string[]) => {
  pivot = [];
  return {
    __group: true,
    leaf: group.leaf,
...
const flattenGroupData = (
  ...
  indexesInGroup: number[] = [],
  groupArray: any[] = [],
  groupCounts: number[] = [],
  pivotUniqueValuesMap: { [key: string]: { [key: string]: any } } = {},
  pivotSummaryValue?: any,
  currentPivotPath?: any[]
) => {
  ...
  const {
    groupItemFactory = createGroupItem,
    isCollapsed = returnFalse,
  } = config;
  ...
  keys.forEach((key: string) => {
    ...
    if (shouldInclude && Array.isArray(newRoot.data)) {
      ...
      let showGroupSummaryRow = normalizeGroupSummary(
        config ? config.showGroupSummaryRow || false : false,
        newRoot
      );
    ...
    let showGroupSummaryRow = normalizeGroupSummary(
      config.showGroupSummaryRow || false,
      newRoot
    );
    ...
    var flattenResult = flattenGroupData(
      newRoot,
      config,
      indexesInGroup,
      groupArray,
      ...
      pivotSummaryValue ? (pivotSummaryValue[key] || {}).data : {},
      currentPivotPath
    );
