...
const err = (msg: string) => {
  console.error(
    `*********************************************** INOVUA REACTDATAGRID ENTERPRISE EDITION ***********************************************
...
export default (props: TypeDataGridProps) => {
  const LICENSE_KEY =
    props.licenseKey ||
    (typeof window !== undefined
      ? (window as any).REACTDATAGRID_LICENSE_KEY
      : '');
  const computedLicenseValid = useMemo(() => {
    ...
    const currentVersionDate = new Date((packageJSON as any).versionTimestamp!);
    const licenseExpiryDate = new Date(licenseInfo.ExpiryDate);

    if (licenseExpiryDate < currentVersionDate) {
      err(
        `Your license key expired on ${licenseInfo.ExpiryDate} but you're trying to use a version published on ${currentVersionDate}`
      );
      return false;
    }
