...
export const getLockedEndWrapperTranslate = ({ virtualizeColumns, lockedEndColumns, availableWidth, }) => {
...
let IDS = 1;
export const updateLockedWrapperPositions = function ({ computedRowExpandEnabled, computedStickyRows, computedFooterRows, computedLockedRows, rowDetailsWidth, lockedStartColumns, lockedEndColumns, virtualListBorderLeft, lockedRows, getScrollLeftMax, rtl, }, scrollLeft, _) {
    let node;
    try {
    ...
    if (computedRowExpandEnabled) {
        const rowContainer = node.querySelector('.inovua-react-virtual-list__row-container');
        if (rowContainer) {
            ...
            const rowDetails = rowContainer.parentElement.querySelectorAll(`[data-id="${dataId}"] > .InovuaReactDataGrid__row > .InovuaReactDataGrid__row-details`);
    ...
    if (lockedRows && lockedRows.length) {
        const lockedRowsContainer = node.parentNode.querySelectorAll('.InovuaReactDataGrid__locked-row');
    ...
    if (lockedStartColumns && lockedStartColumns.length) {
        ...
        if (computedLockedRows && computedLockedRows.length) {
            ...
            const lockedCellsStartRowsContainer = node.parentNode.querySelectorAll('.InovuaReactDataGrid__locked-row-group--start');
        ...
        if (computedFooterRows && computedFooterRows.length) {
            // now select wrapper for locked start cells
            const footerCellsStartRowsContainer = node.parentNode.parentNode.parentNode.querySelectorAll('.InovuaReactDataGrid__footer-row-group--locked-start');
        ...
        if (computedLockedRows && computedLockedRows.length) {
            // now select wrapper for locked start cells
            const lockedRowsCellsStartRowsContainer = node.parentNode.querySelectorAll('.InovuaReactDataGrid__locked-row-group--locked-start');
    ...
    if (lockedEndColumns && lockedEndColumns.length) {
        ...
        const nodesEnd = node.querySelectorAll('.InovuaReactDataGrid__locked-end-wrapper:not(.InovuaReactDataGrid__locked-end-wrapper--sticky)');
        ...
        if (lockedRows && lockedRows.length) {
            // now select wrapper for locked start cells
            const lockedCellsEndRowsContainer = node.parentNode.querySelectorAll('.InovuaReactDataGrid__locked-row-group--end');
        ...
        if (computedFooterRows && computedFooterRows.length) {
            // now select wrapper for locked start cells
            const footerCellsStartRowsContainer = node.parentNode.parentNode.parentNode.querySelectorAll('.InovuaReactDataGrid__footer-row-group--locked-end');
            [].forEach.call(footerCellsStartRowsContainer, el => {
                el.style.transform = lockedEndTransform;
            });
        }
        if (computedLockedRows && computedLockedRows.length) {
            // now select wrapper for locked start cells
            const lockedRowCellsStartRowsContainer = node.parentNode.parentNode.querySelectorAll('.InovuaReactDataGrid__locked-row-group--locked-end');
    ...
    if (computedFooterRows && computedFooterRows.length) {
        const footerScroller = node.parentNode.parentNode.parentNode.querySelector('.InovuaReactDataGrid__footer-rows-container-scroller');
        if (footerScroller) {
